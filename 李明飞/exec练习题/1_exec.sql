# 1. 查找部门编号 30 中员工的详细信息
SELECT * FROM scott.emp WHERE DEPTNO=30;
# 2. 找出从事 clerk 工作的员工的编号、姓名、部门号
SELECT * FROM scott.emp WHERE JOB='clerk';
# 3. 检索出奖金多于基本工资的员工信息
SELECT * FROM scott.emp WHERE COMM>emp.SAL;
# 4. 检索出奖金多于基本工资 30% 员工信息
SELECT * FROM scott.emp WHERE COMM>emp.SAL*0.3;
# 5. 希望看到 10 部门的经理manager或者 20 部门的职员 clerk 的信息
SELECT *
FROM scott.emp
WHERE (DEPTNO = 10 AND JOB = 'manager')
      OR (DEPTNO = 20 AND JOB = 'clerk');
# 6. 找出 10 部门的经理、20 部门的职员或者既不是经理也不是职员但是高于 收入2000 元的员工信息
SELECT *
FROM scott.emp
WHERE (DEPTNO = 10 AND JOB = 'manager')
      OR (DEPTNO = 20 AND JOB = 'clerk')
      OR (JOB <> 'manager' AND JOB <> 'clerk' AND (sal + ifnull(COMM, 0) > 2000));
# 7. 找出获得奖金的员工的工作 去掉重复的
...
FROM scott.emp;
# 17. 返回员工的详细信息并按姓名排序 如果是汉字按uni表排序
SELECT * FROM scott.emp ORDER BY ENAME;
# 18. 返回员工的信息并按员工的工作年限降序排列
SELECT * FROM scott.emp ORDER BY HIREDATE  ;
# 19. 返回员工的信息并按工作降序、工资升序排列  升序是默认的
SELECT * FROM scott.emp ORDER BY JOB DESC ,(SAL+ifnull(COMM,0));
...
# 21. 计算员工的日薪，每月按 30 天  返回参数 x 的四舍五入的有 y 位小数的值
SELECT
  ENAME,
...
# 23. 至今为止，员工被雇佣的天数  datediff 日期差距的天数
SELECT
  hiredate,
...
SELECT * FROM scott.emp WHERE ENAME REGEXP 'a';-- 正则表达式
